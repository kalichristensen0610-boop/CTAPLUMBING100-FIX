...
const schema = z.object({
  ...
  smsTransactionalConsent: z.boolean(), smsMarketingConsent: z.boolean(), source: z.literal("estimate-a"), website: z.string().max(0),
});
...
export async function POST(request: Request) {
  ...
  const parsed=schema.safeParse({name:form.get("name"),phone:form.get("phone"),service:form.get("service"),smsTransactionalConsent:form.get("smsTransactionalConsent")==="true",smsMarketingConsent:form.get("smsMarketingConsent")==="true",source:form.get("source"),website:form.get("website")||""});
