...
const schema = z.object({
  ...
  smsConsent: z.boolean(), source: z.literal("estimate-a"), website: z.string().max(0),
});
...
export async function POST(request: Request) {
  ...
  const parsed=schema.safeParse({name:form.get("name"),phone:form.get("phone"),service:form.get("service"),smsConsent:form.get("smsConsent")==="true",source:form.get("source"),website:form.get("website")||""});
  ...
  try{
    ...
    await transporter.sendMail({from,to:recipient,cc,subject:`Estimate funnel request: ${lead.service}`,text:["Source: estimate-a paid advertising landing page",`Name: ${lead.name}`,`Phone: ${lead.phone}`,`Service: ${lead.service}`,`SMS consent: ${lead.smsConsent?"Yes":"No"}`].join("\n")});
