...
export function Header() {
  ...
  return <div className="site-global-header">
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="container flex h-24 items-center justify-between gap-4">
      ...
      <nav className="hidden items-center gap-3 lg:flex xl:gap-5" aria-label="Main navigation">{nav.map((item) => <Link className="text-xs font-semibold text-slate-700 hover:text-copper-dark xl:text-sm" href={item.href} key={item.href}>{item.label}</Link>)}<Button variant="emergency" asChild><Link href="/emergency-plumbing">Emergency Plumbing</Link></Button></nav>
      <div className="flex items-center gap-2 lg:hidden"><Button variant="emergency" size="icon" asChild><a href={phoneHref} aria-label={`Call ${site.name}`}><Phone /></a></Button><Button variant="outline" size="icon" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="mobile-menu" aria-label="Toggle navigation">{open ? <X /> : <Menu />}</Button></div>
    </div>
    {open && <nav id="mobile-menu" className="border-t bg-white px-4 pb-5 lg:hidden" aria-label="Mobile navigation">{nav.map((item) => <Link onClick={() => setOpen(false)} className="block border-b py-4 font-semibold text-navy" href={item.href} key={item.href}>{item.label}</Link>)}<Button className="mt-4 w-full" variant="emergency" asChild><Link onClick={() => setOpen(false)} href="/emergency-plumbing">Emergency Plumbing</Link></Button></nav>}
    </header>
  </div>;
}
